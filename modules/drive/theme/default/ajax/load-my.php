<?php
	if(isset($_POST['folder']) && !empty($_POST['folder'])) {
		$folder = $drvObj->getItemByStorageName($_POST['folder'], $_SESSION['USERID']);
	}
...
?>
<h3 class="drive-title"><i class="fa fa-archive"></i> <a href="<?php _e($module_url);?>/my">My Documents</a><?php if(isset($folder)) { ?> <i class="fa fa-caret-right"></i> <?php _e($folder['fileName']);?><?php } ?></h3>
<section class="panel">
  <div class="panel-body drive-body">
	<?php
		if(isset($_POST['folder']) && !empty($_POST['folder'])) {
			...
			$items = $drvObj->getDriveByUser($_SESSION['USERID']);
		}
    ...
    <table class="display table" id="table-info" width="100%" style="table-layout:fixed;">
      <thead>
      	<tr class="row">
		  <th class="col-lg-1 _chk"><input type="checkbox" name="checkall" value="1" /></th>
      ...
      <tbody>
		 ...
		 ?>
          <tr class="row" style="display:block;">
          	<td class="col-lg-1" style="text-align:center;"><input type="checkbox" name="checkall" value="1" /></td>
            <td class="col-lg-5">
            	<?php if($item['type'] == 'folder') { ?>
                <a onclick="location.href='javascript:void(0);'" class="ajaxButton loadMore" data-lib="thickBox" data-url="drive/load/my?sort=latest&folder=<?php _e($item['storageName']);?>" data-container="#driveItems" data-action="add" data-location="folder/<?php _e($item['storageName']);?>"><i class="fa fa-<?php _e($item['type']);?>" style="font-size:18px;"></i> <?php _e($item['fileName']);?></a>
                <?php } else { ?>
                	<?php if(strtolower($ext) == 'ods') { ?>
						<a href="<?php _e($module_url.'/xls/'.$item['fileTime']);?>?TB_iframe=true&full=true" class="thickbox <?php _e(strtolower($ext));?>">&nbsp;&nbsp;<?php _e($item['fileName']);?></a>
					<?php } else if(in_array($ext, array_keys($convertTypes)) || strtolower($ext) == 'pdf') { ?>
